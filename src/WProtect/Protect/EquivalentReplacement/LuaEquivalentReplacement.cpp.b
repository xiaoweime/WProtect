...
unsigned long g_table_size = 0;
unsigned long **g_addr_table = NULL;
unsigned long g_table_maxsize = 0;
//unsigned long g_table_maxsize = 0;


unsigned long ip = 0xFFFFFFFF;
unsigned long BUF_MAX_SIZE = 0;
char hexcode[20];
int tempsize = 0;
char tempasm[0x20];
unsigned long g_imagebase = 0;
...
static int lua_Assemble(lua_State *L)
{
	...
	const char *asm_buf = lua_tostring(L,-1);
	t_asmmodel model;
	char perror[256];
	...
	{
		fprintf(stderr,"Assemble指令不正确:%s,原指令%s\r\n",asm_buf,tempasm);
		lua_pushboolean(L,0);
		return 1;
	}

	if (size + g_size > 0xFFFF)  //内存不够了 从新分配
	{
		unsigned char * newasmbuf = new unsigned char[BUF_MAX_SIZE+0xFFFF];
		memcpy(newasmbuf,g_buf,BUF_MAX_SIZE);
		BUF_MAX_SIZE += 0xFFFF;
		delete [] g_buf;
		g_buf = newasmbuf;
	}
	memcpy(&g_buf[g_size],model.code,size);
	g_size += size;
#ifdef DEBUG
	printf("asm:%s\r\n",asm_buf);
#endif
...
{
	memcpy(&g_buf[g_size],hexcode,tempsize);
	g_size += tempsize;
	return 1;
}
...
bool call_lua_fuc(char * fucname)
{
	...
	if (0 != lua_pcall(L,0,0,0))
	{
		...
		fwrite(lua_tostring(L,-1),1,1,file);
...
pCodeInformation lua_code_replacement(pCodeInformation info)
{

	return NULL;
}

void add_table_addr(unsigned long **table,int size)  //扩充tableaddr
{
	unsigned long **newtable = new unsigned long*[g_table_size + size];
	for (int i = 0;i < g_table_size+size;i++)
	{
		newtable[i] = new unsigned long[4];
		if (i < g_table_size)
		{
		   memcpy(newtable[i],g_addr_table[i],4);
		   delete [] g_addr_table[i];
		}
	}
	delete []g_addr_table;
	g_addr_table = newtable;
}

unsigned long get_old_addr(int oldaddr,int table_max_size)
{
	int tablesize = table_max_size ? table_max_size:g_table_size;
	for (int i=0;i<tablesize;i++)
	{
		if (oldaddr == g_addr_table[i][0])
		{
			return g_addr_table[i][1];
		}
	}
	return oldaddr;  //没有找到就返回原来的地址
}
...
static int generate_rand(lua_State *L)
{
...
{
	for (int i = 0 ; i < g_table_maxsize ;i++)
	{
		delete [] g_addr_table[i];
	}
	delete []g_addr_table;
...
void set_buf_addr(unsigned char * buf)  //reloc_jcc_addr 找不到g_buf 无法正确重定位 设置一下
{
	g_buf = buf;
}
void reloc_jcc_addr()
{
	pInstructionBlockList nowlist = get_instruction_block_list_head();
	while (nowlist)
	{
		char jcc[10];
		switch (nowlist->list->udobj.mnemonic)
		{
		case UD_Ija:
			strcpy(jcc,"ja ");
			break;
		...
		{
			char jcc_addr[20];
			if (nowlist->list->udobj.operand[0].type == UD_OP_IMM || nowlist->list->udobj.operand[0].type == UD_OP_JIMM)
			{
				if (nowlist->list->udobj.operand[0].size==8) sprintf(jcc_addr,
					"%s0x%x",
					jcc,
					get_old_addr(nowlist->list->udobj.operand[0].lval.sbyte+nowlist->list->udobj.pc,g_table_size));
				else if (nowlist->list->udobj.operand[0].size==16) sprintf(
					jcc_addr,
					"%s0x%x",
					jcc,
					get_old_addr(nowlist->list->udobj.operand[0].lval.sword+nowlist->list->udobj.pc,g_table_size));
				else if (nowlist->list->udobj.operand[0].size==32) sprintf(
					jcc_addr,
					"%s0x%x",
					jcc,
					get_old_addr(nowlist->list->udobj.operand[0].lval.sdword+nowlist->list->udobj.pc,g_table_size));
				//size = Disasm(jcc_addr,20, nowlist->udobj.insn_offset + diff, &da, DISASM_CODE);  应该是assemble的 弄错了

				t_asmmodel model;
				char perror[256];
				//int size = Assemble(jcc_addr,ip+g_size,&model,0,0,perror);
				int size = Assemble(jcc_addr,get_old_addr(nowlist->list->udobj.insn_offset,g_table_size),&model,0,0,perror);
				if (g_buf == NULL)
				{
					g_buf = new unsigned char [0xFFFF];
					BUF_MAX_SIZE = 0xFFFF;
				}
				memcpy (&g_buf[get_old_addr(nowlist->list->udobj.insn_offset,g_table_size)-g_imagebase],model.code,size);
			}
			else
			{
				memcpy(&g_buf[get_old_addr(nowlist->list->udobj.insn_offset,g_table_size)-g_imagebase],nowlist->list->udobj.inp_sess,nowlist->list->udobj.inp_ctr);
			}
	...
	//g_table_size = 0;
	//g_table_maxsize = 0;
	//for (int n=0;n<g_table_size;n++)
	//{
	//	delete [] g_addr_table[n];
	//}
...
pCodeInformation lua_equivalent_replacement(pCodeInformation info,unsigned long imagebase)
{
	...
	int top = lua_gettop(L);
	int loaderr = luaL_loadfile(L,"wprotect_config.lua");
	if (loaderr)
	{
		error (L,"cannot run config. file: %s\r\n",lua_tostring(L,-1));
		lua_settop(L,top);
	...
	{
		error (L,"call lua error: %s\r\n",lua_tostring(L,-1));
		lua_settop(L,top);
	}
	lua_register(L,"Assemble",lua_Assemble);
	lua_register(L,"OldCode",lua_Oldcode);
	lua_register(L,"Rand",generate_rand);
    //lua_pushcfunction(L,lua_Assemble);  //注册函数
    //lua_setglobal(L,"Assemble");    //注册函数
	...
	unsigned long table_maxsize;
	if (g_addr_table == NULL)
	{
		 table_maxsize = 0xFFF;
		g_addr_table = new unsigned long*[table_maxsize];
		for (int n=0;n<table_maxsize;n++)
		{
			g_addr_table[n] = new unsigned long [4];
			memset(g_addr_table[n],0,4);
		}
		g_table_maxsize = table_maxsize;
	}
	else
		table_maxsize = g_table_maxsize;


	unsigned long table_count = g_table_size;
	while (ud_disassemble(&ud_obj) != 0)
	{
		...
		strcpy(tempasm,ud_obj.insn_buffer);
		memcpy(hexcode,ud_obj.inp_sess,ud_obj.inp_ctr);
		tempsize = ud_obj.inp_ctr;

		g_table_size = table_count;
		if (table_count +  1 > table_maxsize)
		{
			unsigned long **newtable = new unsigned long*[table_maxsize+0xFFF];
			table_maxsize = table_maxsize + 0xFFF;
			for (int i = 0; i < table_maxsize;i++)
			{
				newtable[i] = new unsigned long [4];
				if (i < table_maxsize - 0xFFF)
				{
					memcpy(newtable[i],g_addr_table[i],4);
					delete [] g_addr_table[i];
				}
		...
		g_addr_table[table_count][0] = ud_obj.insn_offset;

		g_addr_table[table_count][1] = ip+g_size;
		table_count ++;
		...
		default:
			{
				...
				add_instruction_block_list(add_instruction_list(list,&ud_obj));g_size+=8; 
				break;
			}
	...
	//				"%s0x%x",
	//				jcc,
	//				get_old_addr(nowlist->list->udobj.operand[0].lval.sbyte+nowlist->list->udobj.pc,table_maxsize));
	//			else if (nowlist->list->udobj.operand[0].size==16) sprintf(
	//				jcc_addr,
	//				"%s0x%x",
	//				jcc,
	//				get_old_addr(nowlist->list->udobj.operand[0].lval.sword+nowlist->list->udobj.pc,table_maxsize));
	//			else if (nowlist->list->udobj.operand[0].size==32) sprintf(
	//				jcc_addr,
	//				"%s0x%x",
	//				jcc,
	//				get_old_addr(nowlist->list->udobj.operand[0].lval.sdword+nowlist->list->udobj.pc,table_maxsize));
	//			//size = Disasm(jcc_addr,20, nowlist->udobj.insn_offset + diff, &da, DISASM_CODE);  应该是assemble的 弄错了

	//			t_asmmodel model;
	//			char perror[256];
	//			//int size = Assemble(jcc_addr,ip+g_size,&model,0,0,perror);
	//			int size = Assemble(jcc_addr,get_old_addr(nowlist->list->udobj.insn_offset,table_maxsize),&model,0,0,perror);
	//			memcpy (&g_buf[get_old_addr(nowlist->list->udobj.insn_offset,table_maxsize)-imagebase],model.code,size);
	//		}
	//		else
	//		{
	//			memcpy(&g_buf[get_old_addr(nowlist->list->udobj.insn_offset,table_maxsize)-imagebase],nowlist->list->udobj.inp_sess,nowlist->list->udobj.inp_ctr);
	//		}
	...
	{
		g_buf[g_size] = 0xE9;
		*(int *)&g_buf[g_size+1] = (int)((info->base + info->size) - (imagebase + g_size) - 5);
		g_size+=5;
	}
...
//	delete [] info->buf;
//	delete info;
	codeinfo->base = imagebase;
	codeinfo->buf = g_buf;
	codeinfo->size = g_size;
	g_buf = NULL;
	g_size = 0;
	//g_addr_table = NULL;
	lua_close(L);
